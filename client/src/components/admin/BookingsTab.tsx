...
import { Ban, CalendarDays, Check, FileSpreadsheet, Loader2, Pencil, Phone, X } from 'lucide-react'
...
function displayStatus(booking: AdminBooking): PillStatus {
...
export function BookingsTab() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-full border border-purple/40 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-purple-bright transition-colors hover:bg-purple/10"
          >
                ...
                {visible.map((booking) => (
                  <Fragment key={booking.id}>
                    <tr className="border-b border-surface-300/20 transition-colors last:border-b-0 hover:bg-surface-100/40">
                      ...
                      <td className="px-5 py-4"><StatusPill status={displayStatus(booking)} /></td>
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          {booking.paymentStatus === 'PENDING' && (
                            <>
                              ...
                              >
                                {busyId === booking.id ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />}
                                Accept
                              </button>
                              <button
                                type="button"
                      ...
                      <tr className="border-b border-surface-300/20 bg-magenta/[0.04]">
                        <td colSpan={7} className="px-5 py-4">
                          <RejectPaymentPanel
                            ...
                            onConfirm={(message) => {
                              setRejectingId(null)
                              void handleReject(booking.id, message)
                            }}
                          />
                        </td>
                      </tr>
      ...
      {showReport && <BookingReportDialog onClose={() => setShowReport(false)} />}
    </div>
  )
}